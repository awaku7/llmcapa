...
INSTALLED = (
    Path(__file__).resolve().parents[1] / "src" / "llmcapa" / "data" / "rekaai.json"
)
        ...
        extra.update(
            {
                ...
                "official_public_models": PUBLIC,
                "models_api": "https://api.reka.ai/v1/models",
            }
        )
        if model.get("model_id") == "rekaai/reka-edge":
            model["aliases"] = sorted(
                set(model.get("aliases", []) + ["reka-edge-2603"])
            )
        updated += 1
    DATA.write_text(
