...
def main() -> None:
    ...
    if len(text) < 1000:
        raise RuntimeError("Tencent official pricing page returned insufficient content")
    ...
    for model in data.get("models", []):
        ...
        model["pricing"] = {"input_per_1m": inp, "output_per_1m": out, "currency": "CNY"}
        extra = model.setdefault("extra", {})
        extra.update({"official_source": SOURCE, "official_source_checked_at": today, "official_spec_refresh": "parsed", "official_product_name": official_name, "pricing_currency": "CNY"})
        updated += 1
