...
def main() -> None:
    with sync_playwright() as pw:
        ...
        for i in range(rows.count()):
            ...
            context_text = cells.nth(3).inner_text().strip() if cells.count() > 3 else ""
            if not model_id:
                continue
            ...
            models.append({
                ...
                "supports_vision": True if multimodal else False,
                "supports_reasoning": True if "gpt-oss" in model_id.lower() else None,
                "supports_responses_api": False,
                "pricing": None,
                "deprecated": False,
                "aliases": [],
                "extra": {"source": SOURCE, "source_type": "official_rendered_page"},
            })
        browser.close()
    if not models:
