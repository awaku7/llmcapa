"""Provider catalog, endpoint, and Structured Outputs updater for ollama."""

"""Update endpoint metadata for the ollama provider only."""
import json
...
def update_endpoints():
    ...
    LOG.write_text(
        ...
        + f"\n## ollama endpoint metadata refresh ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",
        encoding="utf-8",
    )
    print("ollama: endpoint metadata updated", n)


"""Audit Structured Outputs for ollama only."""
from pathlib import Path
...
def update_structured():
    ...
    LOG.write_text(
        LOG.read_text(encoding="utf-8")
        + "\n## ollama Structured Outputs audit ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models; documented={DOCUMENTED}.\n",
        encoding="utf-8",
    )
    print("ollama: structured outputs audited", n)
