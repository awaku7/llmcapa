"""Provider catalog, endpoint, and Structured Outputs updater for z-ai."""

"""Update endpoint metadata for the z-ai provider only."""
import json
from datetime import datetime, timezone
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "src/llmcapa/data/z-ai.json"
...
def update_endpoints():
    ...
    LOG.write_text(
        ...
        + f"\n## z-ai endpoint metadata refresh ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",
        encoding="utf-8",
    )
    print("z-ai: endpoint metadata updated", n)


"""Audit Structured Outputs for z-ai only."""
from pathlib import Path
...
def update_structured():
    ...
    LOG.write_text(
        LOG.read_text(encoding="utf-8")
        + "\n## z-ai Structured Outputs audit ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models; documented={DOCUMENTED}.\n",
        encoding="utf-8",
    )
    print("z-ai: structured outputs audited", n)
