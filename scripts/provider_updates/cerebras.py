"""Update the cerebras catalog from its official model page only."""
import json,re
from datetime import date
from pathlib import Path
from urllib.request import Request,urlopen
ROOT=Path(__file__).resolve().parents[2]
DATA=ROOT/"src/llmcapa/data/cerebras.json"
LOG=ROOT/"provider_update_log.md"
SOURCE='https://inference-docs.cerebras.ai/models/overview'
PATTERN='(?<![\\w/])(?:llama|qwen|deepseek|gpt|zai|qwen)[\\w./:-]+'
def fetch():
...
def update_endpoints():
 ...
 LOG.write_text(LOG.read_text(encoding="utf-8")+f"\n## cerebras endpoint metadata refresh ({date.today()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",encoding="utf-8")
 print("cerebras: endpoint metadata updated",n)
