"""Provider catalog, endpoint, and Structured Outputs updater for writer."""

"""Update endpoint metadata for the writer provider only."""
import json
from datetime import datetime, timezone
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "src/llmcapa/data/writer.json"
LOG = ROOT / "provider_update_log.md"
BASE_URL = "https://api.writer.com/v1"
SOURCE = "https://dev.writer.com/api-reference"
...
def update_endpoints():
    ...
    LOG.write_text(
        ...
        + f"\n## writer endpoint metadata refresh ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",
        encoding="utf-8",
    )
    print("writer: endpoint metadata updated", n)


"""Audit Structured Outputs for writer only."""
from pathlib import Path
...
def update_structured():
    ...
    LOG.write_text(
        LOG.read_text(encoding="utf-8")
        + "\n## writer Structured Outputs audit ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models; documented={DOCUMENTED}.\n",
        encoding="utf-8",
    )
    print("writer: structured outputs audited", n)
