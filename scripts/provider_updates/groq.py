"""Update the groq catalog from its official model page only."""
import json,re
from datetime import date
from pathlib import Path
from urllib.request import Request,urlopen
ROOT=Path(__file__).resolve().parents[2]
DATA=ROOT/"src/llmcapa/data/groq.json"
LOG=ROOT/"provider_update_log.md"
SOURCE='https://console.groq.com/docs/models'
PATTERN='(?<![\\w/])(?:llama|meta-llama|openai|qwen|deepseek|gemma|whisper|canopyml|playai|compound)[\\w./:-]+'
def fetch():
...
def update_endpoints():
 ...
 LOG.write_text(LOG.read_text(encoding="utf-8")+f"\n## groq endpoint metadata refresh ({date.today()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",encoding="utf-8")
 print("groq: endpoint metadata updated",n)
