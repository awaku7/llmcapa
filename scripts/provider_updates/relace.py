"""Provider catalog, endpoint, and Structured Outputs updater for relace."""

"""Update endpoint metadata for the relace provider only."""
import json
from datetime import datetime, timezone
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
DATA = ROOT / "src/llmcapa/data/relace.json"
LOG = ROOT / "provider_update_log.md"
BASE_URL = "https://models.relace.ai"
SOURCE = "https://docs.relace.ai/api-reference/introduction"
...
def update_endpoints():
    ...
    LOG.write_text(
        ...
        + f"\n## relace endpoint metadata refresh ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models.\n",
        encoding="utf-8",
    )
    print("relace: endpoint metadata updated", n)


"""Audit Structured Outputs for relace only."""
from pathlib import Path
...
def update_structured():
    ...
    LOG.write_text(
        LOG.read_text(encoding="utf-8")
        + "\n## relace Structured Outputs audit ({datetime.now(timezone.utc).date()})\n\n- Source: {SOURCE}\n- Updated: {n} models; documented={DOCUMENTED}.\n",
        encoding="utf-8",
    )
    print("relace: structured outputs audited", n)
