...
INSTALLED = (
    Path(__file__).resolve().parents[1] / "src" / "llmcapa" / "data" / "ollama.json"
)
...
def main() -> None:
    ...
    result = {
        "models": sorted(models.values(), key=lambda m: m.get("model_id", "").lower())
    }
    DATA.write_text(
