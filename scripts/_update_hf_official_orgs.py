...
def main() -> None:
    ...
    for provider, org in ORGS.items():
        try:
            ...
            for model in data.get("models", []):
                ...
                extra.update(
                    {
                        "official_source": f"https://huggingface.co/{org}",
                        "official_source_checked_at": today,
                        "official_spec_refresh": "parsed_catalog_only",
                        "official_hf_org": org,
                        "official_hf_model_count": len(models),
                        "official_hf_models_sample": models[:100],
                    }
                )
    ...
    lines.extend(
        f"- {provider}: {count} record(s), {status}"
        for provider, count, status in results
    )
    LOG.write_text(
