...
def main() -> None:
    ...
    for mid, (ctx, max_out, inp, out, reasoning) in RULES.items():
        model = by_id.get(mid)
        if model is None:
            model = {"provider": "aion-labs", "model_id": mid, "display_name": mid, "context_window": ctx, "max_output_tokens": max_out, "input_modalities": ["text"], "output_modalities": ["text"], "supports_function_calling": False, "supports_json_mode": False, "supports_streaming": True, "supports_vision": False, "supports_reasoning": reasoning, "supports_chat_completion": True, "supports_responses_api": False, "supports_reasoning_effort": False, "supports_thinking_budget": False, "supports_anthropic_api": False, "supports_google_api": False, "supports_fim": False, "tokenizer_name": "Other", "knowledge_cutoff": None, "deprecated": mid.endswith("2.5"), "aliases": [mid], "license_type": "api", "pricing": {}}
            data["models"].append(model)
            by_id[mid] = model
        model.update({"context_window": ctx, "max_output_tokens": max_out, "pricing": {"input_per_1m": inp, "output_per_1m": out, "currency": "USD"}, "license_type": "api"})
        extra = model.setdefault("extra", {})
        extra.update({"official_source": SOURCE, "official_source_checked_at": today, "official_spec_refresh": "parsed", "official_models_api": "https://api.aionlabs.ai/v1/models"})
        updated += 1
