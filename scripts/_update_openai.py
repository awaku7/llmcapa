...
ENDPOINT = {"base_url": "https://api.openai.com/v1", "protocol": "openai-compatible", "auth": "bearer", "source": "https://platform.openai.com/docs/api-reference"}
...
def detail(path: str) -> dict:
    ...
    input_modalities = [x.strip().lower() for x in re.split(r"[,/]", inp.group(1))] if inp else ["text"]
    output_modalities = [x.strip().lower() for x in re.split(r"[,/]", out.group(1))] if out else ["text"]
    features = set(re.findall(r"^- ([a-z_]+)\s*$", text, re.I | re.M))
    def supported(name: str) -> bool:
        m = re.search(rf"\|\s*{re.escape(name)}[^|]*\|\s*[^|]*\|\s*(Supported|Not supported)", text, re.I)
        return bool(m and m.group(1).lower() == "supported")
    aliases = re.findall(r"^- `([^`]+)`$", text[text.find("## Snapshots"):], re.M) if "## Snapshots" in text else []
    extra = {"source": BASE + path, "endpoints": [ENDPOINT]}
    default = re.search(r"Default snapshot:\s*`([^`]+)`", text, re.I)
    if default:
        extra["default_snapshot"] = default.group(1)
    if "requires separate approval" in text.lower() or "required approval" in text.lower():
        extra["required_approval"] = True
    entry = {"provider": "openai", "model_id": mid, "display_name": re.search(r"^#\s+(.+)$", text, re.M).group(1).strip() if re.search(r"^#\s+(.+)$", text, re.M) else mid, "context_window": line(r"([\d,]+) context window"), "max_output_tokens": line(r"([\d,]+) max output tokens"), "input_modalities": input_modalities, "output_modalities": output_modalities, "deprecated": "deprecated" in text[:1000].lower(), "aliases": aliases, "extra": extra}
    ...
    entry["supports_reasoning"] = "reasoning token support" in text.lower() or "reasoning" in features
    cutoff = re.search(r"([A-Z][a-z]{2} \d{1,2}, \d{4}) knowledge cutoff", text)
    if cutoff: entry["knowledge_cutoff"] = cutoff.group(1)
    return entry


def prices(markdown: str) -> dict[str, tuple[float, float]]:
    result = {}
    for row in re.findall(r"^\|\s*([^|]+?)\s*\|\s*\$?([\d.]+)[^|]*\|[^|]*\|\s*[^|]*\|\s*\$?([\d.]+)", markdown, re.M):
...
def main() -> None:
    ...
    links = re.findall(r"^- \[[^\]]+\]\((/api/docs/models/[^)]+\.md)\)", index, re.M)
    ...
    for path in dict.fromkeys(links):
        ...
        if rate:
            i, o = rate; entry["pricing"] = {"input_per_1m": i, "output_per_1m": o, "currency": "USD"}
        else:
            entry["pricing"] = {"input_per_1m": None, "output_per_1m": None, "currency": "USD"}
        updated.append(entry); seen.add(mid)
    for old in current.get("models", []):
        if old.get("model_id") not in seen: updated.append(old)
