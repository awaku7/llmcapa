...
INSTALLED = (
    Path(__file__).resolve().parents[1] / "src" / "llmcapa" / "data" / "cohere.json"
)
...
def main() -> None:
        ...
        for model in data.get("models", []):
            ...
            if (
                ...
                if mid in PRICE_RULES:
                    ...
                    extra["official_input_per_1m"] = inp
                    extra["official_output_per_1m"] = outp
                    extra["official_spec_refresh"] = (
                        "parsed_numeric_explicit_price_rule"
                    )
            checked += 1
    ...
    print(
        f"cohere.json: official_pages_checked={checked} provenance_entries_changed={changed}"
    )
