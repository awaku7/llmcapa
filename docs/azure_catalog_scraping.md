...
1. 「Next」クリック → SSR 状態から生成された continuationToken 付きで API 呼び出し
1. API → 2ページ目のデータ + 新しい continuationToken を返す
1. 以下繰り返し
...
1. SSR の DOM から1ページ目のモデル名を取得
1. 「Next」ボタンを繰り返しクリック
1. 各クリック後に API レスポンスをキャプチャ
1. API で取得したモデル + SSR のみのモデル（両方に現れないもの）を統合
1. SSR のみのモデルは詳細ページを個別訪問してメタデータを取得
